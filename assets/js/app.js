...
function vocabularyRow(w) {
  ...
  return `<tr style="cursor:pointer" data-action="word-detail" data-word-id="${e(w.id)}"><td><span class="vocab-word"><b>${e(w.word)}</b><small>${e(w.partOfSpeech || 'word')}${w.pronunciation ? ` · ${e(w.pronunciation)}` : ''}</small></span></td><td><strong>${e(senses.primary)}</strong><br><span class="vocab-context">${e(contextMeaning)}</span>${senses.senses.length ? `<div class="vocab-sense-summary">${senses.senses.slice(0, 2).map((sense) => `<span class="sense-chip">${e(sense.meaning)}</span>`).join('')}${senses.extraCount ? `<span class="sense-chip more">+${senses.extraCount}</span>` : ''}</div>` : ''}</td><td>${examples.length ? `<span class="vocab-example-preview"><b>${e(examples[0].sentence)}</b><small>${e(examples[0].translation)}</small><em>AI 예문 ${examples.length}개</em></span>` : `<span class="badge warning">AI 예문 정리 대기</span><br><span title="${e(recentSentence)}">${e(recentSentence.slice(0,34))}${recentSentence.length>34?'…':''}</span>`}</td><td>${w.exposureCount || 1}회</td><td><div class="mastery-bar"><div class="progress-track"><div class="progress-fill" style="width:${w.masteryScore || 0}%"></div></div><b>${w.masteryScore || 0}</b></div></td><td>${formatDate(w.nextReviewAt || todayISO())}</td><td><span class="badge ${statusClass(w.status || '신규')}">${e(w.status || '신규')}</span></td></tr>`;
}
